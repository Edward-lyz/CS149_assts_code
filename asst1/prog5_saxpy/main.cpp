...
// return GB/s
static float
toBW(long int bytes, float sec) {
    return static_cast<float>(bytes) / (1024. * 1024. * 1024.) / sec;
}

static float
toGFLOPS(long int ops, float sec) {
    return static_cast<float>(ops) / 1e9 / sec;
}

static void verifyResult(long int N, float* result, float* gold) {
    for (int i=0; i<N; i++) {
        if (result[i] != gold[i]) {
...
int main() {
    ...
    const unsigned long int N = 200 * 1000 * 2000; // 20 M element vectors (~80 MB)
    const unsigned  long int TOTAL_BYTES = 4 * N * sizeof(float);
    const unsigned long  int TOTAL_FLOPS = 2 * N;
