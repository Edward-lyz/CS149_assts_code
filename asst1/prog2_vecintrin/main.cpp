...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  //
  // TODO: Implement your vectorized version of
  ...
  int _iteration = N/VECTOR_WIDTH;
  int _rem = N%VECTOR_WIDTH;
  for(int i=0;i<_iteration;++i){
    __cs149_mask mask_need_mul;
    __cs149_vec_float res = _cs149_vset_float(1.f);
    int step = i*VECTOR_WIDTH;
    // All ones, default to execute every element
    mask_all = _cs149_init_ones();
    // All zeros, default to execute nothing
    mask_need_mul = _cs149_init_ones(0);
    // load data
    _cs149_vload_float(x,values+step,mask_all);
    _cs149_vload_int(y,exponents+step,mask_all);
    // note that, both x and y are no less than 0
    //find y that greater than 0
    _cs149_vgt_int(mask_need_mul,y,zero,mask_all);
    //compute the exp(x) by mul
    while(_cs149_cntbits(mask_need_mul)){
      _cs149_vmult_float(res,res,x,mask_need_mul);
      _cs149_vsub_int(y,y,one,mask_need_mul);
      _cs149_vgt_int(mask_need_mul,y,zero,mask_need_mul);
    }
    __cs149_mask clampMask;
    _cs149_vgt_float(clampMask, res, upper_bound, mask_all);
    _cs149_vset_float(res, 9.999999f, clampMask);
    _cs149_vstore_float(output + step, res, mask_all);
  }
  // cauculate the tail part
  if(_rem!=0){
    int step_tail = _iteration*VECTOR_WIDTH;
    clampedExpSerial(values+step_tail,exponents+step_tail,output+step_tail,N-step_tail);
  }
